...
import { authenticateToken, checkDocumentAccess, } from "../features/auth/middleware/auth.js";
import { getCurrentDateTime } from "../lib/dayjs.js";
import { getPDFSignedUrl } from "../features/common/services/s3Service.js";
const router = Router();
router.get("/:documentType/:documentId", authenticateToken, checkDocumentAccess, async (req, res) => {
    try {
...
router.get("/generate-url/:documentType/:documentId", authenticateToken, checkDocumentAccess, async (req, res) => {
    try {
        const { documentType, documentId } = req.params;
        let token = null;
        ...
        try {
            ...
            const secureUrl = `${req.protocol}://${req.get("host")}/pdf/${documentType}/${documentId}?token=${token}`;
            res.json({
                success: true,
...
//# sourceMappingURL=pdfAccess.js.map