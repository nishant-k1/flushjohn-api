...
router.get("/", async function (req, res) {
    try {
        const { page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", search = "", searchQuery = "", startDate = null, endDate = null, page: _page, limit: _limit, sortBy: _sortBy, sortOrder: _sortOrder, ...columnFilters } = req.query;
...
router.post("/:id/pdf", validateAndRecalculateProducts, async function (req, res) {
    try {
        ...
        const { generateSalesOrderPDF } = await import("../../fileManagement/services/pdfService.js");
...
router.post("/:id/email", validateAndRecalculateProducts, async function (req, res) {
    try {
        ...
        const { generateSalesOrderPDF } = await import("../../fileManagement/services/pdfService.js");
        const { sendSalesOrderEmail, sendInvoiceEmail } = await import("../../common/services/emailService.js");
        let pdfUrls;
        try {
        ...
        await salesOrdersService.linkSalesOrderToCustomer(updatedSalesOrder, updatedSalesOrder.lead?.toString() || null);
        res.status(200).json({
            success: true,
