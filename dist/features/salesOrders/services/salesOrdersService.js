    ...
    while (attempts < maxRetries) {
        try {
            ...
            if (attempts >= maxRetries) {
                throw new Error("Failed to generate unique sales order number after retries");
            }
            await new Promise((resolve) => setTimeout(resolve, 50 * attempts));
        }
    }
...
export const createSalesOrder = async (salesOrderData) => {
    ...
    if (createdSalesOrder.lead) {
        try {
            await conversationLogRepository.updateOnSalesOrderCreated(createdSalesOrder.lead, createdSalesOrder._id);
        }
        catch (error) {
            // Log but don't fail the sales order creation
            console.error("Error updating ConversationLog on SalesOrder creation:", error);
        }
    }
    ...
    try {
        ...
        // Log but don't fail the sales order creation
        console.error("Error creating/linking customer on SalesOrder creation:", error);
    }
    ...
    try {
        ...
        // Log but don't fail the sales order creation
        console.error("Error updating payment totals on SalesOrder creation:", error);
    }
    // Fetch updated sales order to return with correct totals
    const updatedSalesOrder = await salesOrdersRepository.findById(createdSalesOrder._id);
 ...
 */
const getAllSalesOrdersWithAggregation = async ({ page, limit, sortBy, sortOrder, search, startDate, endDate, columnFilters, }) => {
    ...
    if (search && search.trim()) {
        ...
        searchConditions.push({ customerName: { $regex: escapedSearch, $options: "i" } }, { customerEmail: { $regex: escapedSearch, $options: "i" } }, { customerPhone: { $regex: escapedSearch, $options: "i" } }, { eventLocation: { $regex: escapedSearch, $options: "i" } }, { deliveryDate: { $regex: escapedSearch, $options: "i" } }, { pickupDate: { $regex: escapedSearch, $options: "i" } }, { emailStatus: { $regex: escapedSearch, $options: "i" } }, { contactPersonName: { $regex: escapedSearch, $options: "i" } }, { contactPersonPhone: { $regex: escapedSearch, $options: "i" } }, { instructions: { $regex: escapedSearch, $options: "i" } }, { note: { $regex: escapedSearch, $options: "i" } }, { status: { $regex: escapedSearch, $options: "i" } }, { paymentStatus: { $regex: escapedSearch, $options: "i" } });
...
export const getAllSalesOrders = async ({ page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", search = "", startDate = null, endDate = null, ...columnFilters }) => {
    const skip = (page - 1) * limit;
...
export const getSalesOrderById = async (id) => {
    ...
    const salesOrderObj = salesOrder.toObject
        ? salesOrder.toObject()
        : salesOrder;
    const { calculateOrderTotal } = await import("../../payments/services/stripeService.js");
    const calculatedOrderTotal = calculateOrderTotal(salesOrderObj.products || []);
    // Check if recalculation is needed: orderTotal doesn't match calculated total, or balanceDue is 0 when it shouldn't be
    const needsRecalculation = salesOrderObj.orderTotal !== calculatedOrderTotal ||
    ...
    if (needsRecalculation) {
        try {
            ...
            return formatSalesOrderResponse(updatedSalesOrder || salesOrder, (updatedSalesOrder || salesOrder).lead);
        }
        catch (error) {
...
export const updateSalesOrder = async (id, updateData) => {
    ...
    Object.keys(salesOrderFields).forEach((key) => salesOrderFields[key] === undefined && delete salesOrderFields[key]);
    ...
    if (updateData.products !== undefined) {
        try {
            ...
            console.error("Error updating payment totals on SalesOrder update:", error);
        }
    }
    const updatedSalesOrder = await salesOrdersRepository.findById(id);
    return formatSalesOrderResponse(updatedSalesOrder || salesOrder, updatedSalesOrder?.lead);
};
export const cancelSalesOrder = async (id) => {
    ...
    if (hasUnrefundedPayments) {
        const error = new Error("Cannot cancel sales order. Please refund all payments before cancelling.");
...
export const deleteSalesOrder = async (id) => {
    ...
    if (jobOrdersCount > 0) {
        const error = new Error(`Cannot delete sales order. Related records exist: ${jobOrdersCount} job order(s). ` +
...
export const linkSalesOrderToCustomer = async (salesOrder, leadId = null) => {
    ...
    if (existingCustomer) {
        await customersRepository.findOneAndUpdate({ email: salesOrder.email }, {
            $addToSet: {
...
//# sourceMappingURL=salesOrdersService.js.map