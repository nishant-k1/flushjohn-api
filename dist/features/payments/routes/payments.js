 ...
 */
router.post("/sales-orders/:salesOrderId/create-payment-link", async function (req, res) {
    try {
        ...
        const result = await paymentsService.createSalesOrderPaymentLink(salesOrderId, returnUrl);
        res.status(201).json({
            success: true,
 ...
 */
router.post("/sales-orders/:salesOrderId/save-payment-method", async function (req, res) {
    try {
        const { salesOrderId } = req.params;
...
router.get("/customers/:customerId/payment-methods", async function (req, res) {
    try {
        ...
        const paymentMethods = await paymentsService.getCustomerPaymentMethods(customerId);
        res.status(200).json({
            success: true,
 ...
 * IMPORTANT: Specific routes must come BEFORE the catch-all /:paymentId route
 */
router.delete("/customers/:customerId/payment-methods/:paymentMethodId", async function (req, res) {
    try {
...
router.get("/sales-orders/:salesOrderId", async function (req, res) {
    try {
        ...
        const payments = await paymentsService.getPaymentsBySalesOrder(salesOrderId);
        res.status(200).json({
            success: true,
            data: payments,
        });
...
router.post("/:paymentId/cancel", async function (req, res) {
    try {
        const { cancelPaymentLink } = await import("../services/paymentsService.js");
        const { paymentId } = req.params;
        if (!isValidObjectId(paymentId)) {
            return res.status(400).json({
                success: false,
...
router.post("/:paymentId/sync", async function (req, res) {
    try {
        ...
        const { syncPaymentLinkStatus } = await import("../services/paymentsService.js");
        ...
        if (error.message.includes("Only payment link payments") ||
...
router.post("/:paymentId/refund", async function (req, res) {
    try {
        ...
        const result = await paymentsService.refundPayment(paymentId, amount, reason);
        res.status(200).json({
            success: true,
