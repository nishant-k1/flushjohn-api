 ...
 */
export const createPaymentLink = async ({ amount, currency = "usd", description, metadata = {}, customerEmail, customerName, returnUrl, }) => {
    try {
        const stripe = getStripeClient();
 ...
 */
export const createPaymentIntent = async ({ amount, currency = "usd", customerId, paymentMethodId, description, metadata = {}, savePaymentMethod = false, }) => {
    try {
        const stripe = getStripeClient();
        const amountInCents = amountToCents(amount);
...
export const attachPaymentMethodToCustomer = async (paymentMethodId, customerId) => {
    try {
        ...
        try {
            ...
            const isAlreadyAttached = existingMethods.data.some((pm) => pm.id === paymentMethodId);
 ...
 * Process Refund
 */
export const processRefund = async ({ chargeId, paymentIntentId = null, // Optional: can refund via payment intent if charge ID not available
