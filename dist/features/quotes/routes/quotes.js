...
router.get("/", async function (req, res) {
    try {
        const { page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", search = "", searchQuery = "", page: _page, limit: _limit, sortBy: _sortBy, sortOrder: _sortOrder, ...columnFilters } = req.query;
...
router.get("/ai-suggested-rate", async function (req, res) {
    try {
        const { zipCode, city, state, streetAddress, productItem, quantity, usageType, } = req.query;
        if (!zipCode && !city) {
            return res.status(400).json({
                success: false,
...
router.post("/:id/pdf", validateAndRecalculateProducts, async function (req, res) {
    try {
        ...
        const { generateQuotePDF } = await import("../../fileManagement/services/pdfService.js");
...
router.post("/:id/email", validateAndRecalculateProducts, async function (req, res) {
    try {
        ...
        const { generateQuotePDF } = await import("../../fileManagement/services/pdfService.js");
        const { sendQuoteEmail } = await import("../../common/services/emailService.js");
        let pdfUrls;
        try {
