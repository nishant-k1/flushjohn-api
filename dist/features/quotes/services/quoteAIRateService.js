...
const getOpenAIClient = () => {
    if (!openai) {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error("OPENAI_API_KEY environment variable is required for AI rate suggestions");
        }
        openai = new OpenAI({
 ...
 */
export const getAISuggestedRate = async ({ zipCode, city, state, streetAddress, productItem, quantity = 1, usageType, }) => {
    try {
        if (!process.env.OPENAI_API_KEY) {
        ...
        try {
            ...
            if (jobOrders && jobOrders.length > 0) {
                historicalData.jobOrders = jobOrders
                    ...
                    .map((jo) => {
                    const matchingProduct = jo.products.find((p) => p.item &&
                        productItem &&
        ...
        try {
            ...
            if (salesOrders && salesOrders.length > 0) {
                historicalData.salesOrders = salesOrders
                    ...
                    .map((so) => {
                    const matchingProduct = so.products.find((p) => p.item &&
                        productItem &&
                        p.item.toLowerCase().includes(productItem.toLowerCase()));
                    if (matchingProduct) {
                        return {
        ...
        try {
            ...
            if (quotes && quotes.length > 0) {
                historicalData.quotes = quotes
                    ...
                    .map((q) => {
                    const matchingProduct = q.products.find((p) => p.item &&
                        productItem &&
                        p.item.toLowerCase().includes(productItem.toLowerCase()));
                    if (matchingProduct) {
                        return {
        ...
        const totalSamples = historicalData.vendorPricing.length +
...
${averageVendorCost
            ? `Average Vendor Cost from Historical Data: $${averageVendorCost.toFixed(2)} per unit`
...
2. Regional rate factors (cost of living, market rates for ${city || state || "this area"})
...
Return ONLY valid JSON, no markdown, no code blocks.`;
        ...
        return {
            suggestedRatePerUnit: Math.round(aiResponse.suggestedRatePerUnit * 100) / 100,
...
//# sourceMappingURL=quoteAIRateService.js.map