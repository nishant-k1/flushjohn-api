    ...
    while (attempts < maxRetries) {
        try {
            ...
            if (attempts >= maxRetries) {
                throw new Error("Failed to generate unique job order number after retries");
            }
            await new Promise((resolve) => setTimeout(resolve, 50 * attempts));
        }
    }
...
export const createJobOrder = async (jobOrderData) => {
    ...
    if (createdJobOrder.lead) {
        try {
            await conversationLogRepository.updateOnJobOrderCreated(createdJobOrder.lead, createdJobOrder._id);
        }
        catch (error) {
            // Log but don't fail the job order creation
            console.error("Error updating ConversationLog on JobOrder creation:", error);
        }
    }
 ...
 */
const getAllJobOrdersWithAggregation = async ({ page, limit, sortBy, sortOrder, search, startDate, endDate, }) => {
    ...
    searchConditions.push({ jobLocation: { $regex: escapedSearch, $options: "i" } }, { deliveryDate: { $regex: escapedSearch, $options: "i" } }, { pickupDate: { $regex: escapedSearch, $options: "i" } }, { emailStatus: { $regex: escapedSearch, $options: "i" } }, { vendorAcceptanceStatus: { $regex: escapedSearch, $options: "i" } }, { contactPersonName: { $regex: escapedSearch, $options: "i" } }, { contactPersonPhone: { $regex: escapedSearch, $options: "i" } }, { instructions: { $regex: escapedSearch, $options: "i" } }, { note: { $regex: escapedSearch, $options: "i" } }, { status: { $regex: escapedSearch, $options: "i" } }, { "vendor.name": { $regex: escapedSearch, $options: "i" } });
...
export const getAllJobOrders = async ({ page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", search = "", startDate = null, endDate = null, ...columnFilters }) => {
    const skip = (page - 1) * limit;
...
export const updateJobOrder = async (id, updateData) => {
    ...
    Object.keys(jobOrderFields).forEach((key) => jobOrderFields[key] === undefined && delete jobOrderFields[key]);
    ...
    return formatJobOrderResponse(updatedJobOrder || jobOrder, updatedJobOrder?.lead);
};
...
export const createOrLinkCustomerFromJobOrder = async (jobOrder) => {
    ...
    const SalesOrders = (await import("../../salesOrders/models/SalesOrders.js")).default;
...
//# sourceMappingURL=jobOrdersService.js.map