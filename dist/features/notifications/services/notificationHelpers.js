...
export const createLeadNotification = async (lead) => {
    try {
        ...
        const fullName = `${lead.firstName || lead.fName || ""} ${lead.lastName || lead.lName || ""}`.trim();
        ...
        const notificationPromises = users.map((user) => notificationsService.createOrUpdateNotification(user._id, lead._id, {
            type: "new_lead",
