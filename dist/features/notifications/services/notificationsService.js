...
export const getUserNotifications = async (userId, options = {}) => {
    try {
        const notifications = await notificationsRepository.findByUserId(userId, options);
...
export const createOrUpdateNotification = async (userId, leadId, data) => {
    try {
        const notification = await notificationsRepository.createOrUpdate(userId, leadId, data);
        return notification;
    }
    catch (error) {
...
export const markNotificationAsRead = async (notificationId, userId) => {
    try {
        const notification = await notificationsRepository.markAsRead(notificationId, userId);
        if (!notification) {
...
export const deleteNotification = async (notificationId, userId) => {
    try {
        const result = await notificationsRepository.deleteById(notificationId, userId);
        if (!result) {
            throw new Error("Notification not found or unauthorized");
        }
        return result;
