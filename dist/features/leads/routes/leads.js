...
import { validateCreateLead, validateUpdateLead, validateLeadId, validateGetLeads, handleValidationErrors, } from "../validators/leadValidator.js";
import { RESOURCES, canCreate, canRead, canUpdate, canDelete, } from "../../auth/middleware/permissions.js";
const router = Router();
...
router.post("/", authenticateToken, canCreate(RESOURCES.LEADS), validateCreateLead, handleValidationErrors, validateAndRecalculateProducts, async function (req, res, next) {
    try {
...
router.get("/", authenticateToken, canRead(RESOURCES.LEADS), validateGetLeads, handleValidationErrors, async function (req, res, next) {
    try {
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
        const sortBy = req.query.sortBy || "createdAt";
        const sortOrder = req.query.sortOrder || "desc";
        const { status, assignedTo, leadSource, search, hasCustomerNo, createdAtStart, createdAtEnd, deliveryDateStart, deliveryDateEnd, pickupDateStart, pickupDateEnd, page: _page, limit: _limit, sortBy: _sortBy, sortOrder: _sortOrder, ...columnFilters } = req.query;
...
router.get("/:id", authenticateToken, canRead(RESOURCES.LEADS), validateLeadId, handleValidationErrors, async function (req, res, next) {
    try {
        const { id } = req.params;
...
router.put("/:id", authenticateToken, canUpdate(RESOURCES.LEADS), validateUpdateLead, handleValidationErrors, validateAndRecalculateProducts, async function (req, res, next) {
    try {
        const { id } = req.params;
...
router.put("/update/:id", authenticateToken, validateAndRecalculateProducts, async function (req, res, next) {
    try {
        const { id } = req.params;
        if (!leadsService.isValidObjectId(id)) {
            return res.status(400).json({
...
router.delete("/:id", authenticateToken, canDelete(RESOURCES.LEADS), validateLeadId, handleValidationErrors, async function (req, res, next) {
    try {
        const { id } = req.params;
