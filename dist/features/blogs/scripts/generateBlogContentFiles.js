...
async function generateBlogContent(postData, templateType = "citySpecific") {
    try {
        const content = await blogGeneratorService.generateBlogContent(templateType, templateType === "citySpecific"
        ...
        const metaDescription = await blogGeneratorService.generateMetaDescription(postData.title, content, postData.keywords);
        const slug = blogGeneratorService.generateSlug(postData.title);
        const excerpt = blogGeneratorService.generateExcerpt(content);
        const tags = blogGeneratorService.extractTags(postData.title, content, postData.city || null);
        const coverImageAlt = blogGeneratorService.generateCoverImageAlt(postData.title, postData.city || null);
        ...
        if (!postData.city || !postData.state) {
            extractedLocation = blogGeneratorService.extractCityAndState(postData.title, content);
        }
        const blogPostData = {
...
//# sourceMappingURL=generateBlogContentFiles.js.map