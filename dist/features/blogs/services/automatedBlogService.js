...
export async function generateAutomatedBlogPost(contentType = null, randomize = false) {
    try {
        ...
        const generatedContent = await blogGeneratorService.generateBlogContent(topic.templateType || "citySpecific", topic.templateType === "citySpecific"
        ...
        const [metaDescription, comprehensiveMetadata, coverImageDescription] = await Promise.all([
            blogGeneratorService.generateMetaDescription(topic.title, generatedContent, keywordsObj),
            blogGeneratorService.generateComprehensiveBlogMetadata(topic.title, generatedContent, keywordsObj, topic.category),
            blogGeneratorService.generateCoverImageDescription(topic.title, topic.category, generatedContent),
        ]);
        ...
        const coverImageSrc = await generateAICoverImage(topic.title, topic.category, contentWithLinks);
        ...
        if (!topic.city || !topic.state) {
            extractedLocation = await blogGeneratorService.extractCityAndState(topic.title, contentWithLinks);
        }
        // Generate FAQ schema for better search visibility
        const faqSchema = blogGeneratorService.generateFAQSchema(contentWithLinks, topic.title);
        const blogData = {
            title: topic.title,
...
export async function getAutomationStats() {
    try {
        ...
        const recentAutomatedPosts = automatedPosts.blogs?.filter((post) => post.automated && new Date(post.automationDate) >= thirtyDaysAgo) || [];
        return {
...
//# sourceMappingURL=automatedBlogService.js.map