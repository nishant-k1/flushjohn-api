...
import { runAutomatedBlogGeneration, getAutomationStats, } from "./automatedBlogService.js";
const CRON_CONFIG = {
...
async function weeklyBlogGenerationJob() {
    ...
    try {
        ...
        console.error(`❌ Monday construction blog generation failed:`, error.message);
    }
    finally {
...
async function midWeekBlogGenerationJob() {
    ...
    try {
        ...
        jobStatus[jobName].failedRuns++;
        console.error(`❌ Wednesday city-specific blog generation failed:`, error.message);
    }
    finally {
        jobStatus[jobName].isRunning = false;
        const duration = new Date() - startTime;
...
async function weeklyProblemSolvingJob() {
    ...
    try {
        ...
        jobStatus[jobName].failedRuns++;
        console.error(`❌ Friday problem-solving blog generation failed:`, error.message);
    }
    finally {
        jobStatus[jobName].isRunning = false;
        const duration = new Date() - startTime;
...
export function initializeCronJobs() {
    const weeklyJob = cron.schedule(CRON_CONFIG.weeklyBlogGeneration, weeklyBlogGenerationJob, {
        scheduled: false,
        timezone: "America/New_York",
    });
    const midWeekJob = cron.schedule(CRON_CONFIG.midWeekBlogGeneration, midWeekBlogGenerationJob, {
        scheduled: false,
        timezone: "America/New_York",
    });
    const problemSolvingJob = cron.schedule(CRON_CONFIG.weeklyProblemSolving, weeklyProblemSolvingJob, {
        scheduled: false,
        timezone: "America/New_York",
    });
    const dailyJob = cron.schedule(CRON_CONFIG.dailyStatusCheck, dailyStatusCheckJob, {
        scheduled: false,
        timezone: "America/New_York",
    });
