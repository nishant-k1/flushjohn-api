...
const getOpenAIClient = () => {
    if (!openai) {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error("OPENAI_API_KEY environment variable is required for blog services");
        }
        openai = new OpenAI({
    ...
    try {
        ...
        const response = await getOpenAIClient().chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are an expert content writer. Generate a compelling, SEO-friendly excerpt (summary) for blog posts. The excerpt should be 120-150 characters, engaging, and capture the main value proposition. Return ONLY plain text without any HTML tags, quotes, backticks, or special formatting.",
                },
                {
                    role: "user",
...
export const createBlogWithRetry = async (blogData, maxRetries = 3) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const transformedBlogData = await transformBlobUrls(blogData, createdBlog._id.toString());
            if (transformedBlogData !== blogData) {
                return await blogsRepository.updateById(createdBlog._id, transformedBlogData);
            }
            return createdBlog;
        }
        catch (error) {
...
const transformBlobUrls = async (blogData, blogId = null) => {
    if (blogData.coverImage &&
        ...
        blogId) {
        try {
            const s3Url = await uploadCoverImageToS3(blogId, blogData.coverImageFileType || "image/jpeg", blogData.coverImageFileData);
            const { coverImageFileData, coverImageFileName, coverImageFileType, ...cleanBlogData } = blogData;
            return {
                ...cleanBlogData,
                coverImageS3: {
                    src: s3Url,
...
export const createBlog = async (blogData) => {
    ...
    if ((!blogData.city || !blogData.state) && blogData.content) {
        try {
            const extractedLocation = extractCityAndState(blogData.title, blogData.content);
    ...
    if (!blogData.tags || !blogData.author || !blogData.coverImage?.alt) {
        try {
            const comprehensiveMetadata = await generateComprehensiveBlogMetadata(blogData.title, blogData.content, { primary: blogData.title, secondary: "" }, blogData.category || "tips");
            // Only set if not already provided
            if (!blogData.tags)
                blogData.tags = comprehensiveMetadata.tags;
            if (!blogData.author)
                blogData.author = comprehensiveMetadata.author;
...
export const getAllBlogs = async ({ page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", slug = null, search = "", status = null, ...columnFilters }) => {
...
export const updateBlog = async (id, updateData) => {
    ...
    if (!transformedUpdateData.excerpt && transformedUpdateData.content) {
        // Use AI to generate excerpt
        transformedUpdateData.excerpt = await generateAIExcerpt(transformedUpdateData.content, transformedUpdateData.title || existingBlog.title);
    }
