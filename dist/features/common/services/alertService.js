...
class AlertService {
    ...
    formatTelegramMessage(lead) {
        const { leadNo, leadSource, fName, lName, companyName, email, phone, leadStatus, assignedTo, products, createdAt, } = lead;
        ...
        if (products && products.length > 0) {
            ...
            products.forEach((product, index) => {
                productsText += `${index + 1}. ${escapeMarkdown(product.name)} (Qty: ${product.qty}) - $${product.rate} = $${product.amount}\n`;
            });
        }
...
🔗 CRM Link: View Lead #${leadNo}`;
    ...
    async testConnection() {
        ...
        if (this.telegramEnabled) {
            try {
                const testMessage = "🧪 FlushJohn CRM Test Message\n\nTelegram alerts are working correctly! ✅";
