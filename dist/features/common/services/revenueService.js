 ...
 */
export const calculateRevenue = async ({ startDate, endDate, vendorTransactionCharges = 0, vendorTransactionChargesMode = "percentage", googleAdsSpending = 0, facebookAdsSpending = 0, instagramAdsSpending = 0, linkedInAdsSpending = 0, othersExpenses = 0, }) => {
    try {
        // Format dates
        ...
        while (hasMoreSales) {
            ...
            if (salesResult.data && salesResult.data.length > 0) {
                ...
                const paidSalesOrders = salesResult.data.filter((order) => order.paymentStatus === "Paid");
        ...
        while (hasMoreJobs) {
            ...
            if (jobResult.data && jobResult.data.length > 0) {
                ...
                const sentJobOrders = jobResult.data.filter((order) => order.emailStatus === "Sent");
        ...
        allJobOrders.forEach((jobOrder) => {
            if (jobOrder.salesOrder) {
                const salesOrderId = typeof jobOrder.salesOrder === "string"
                    ? jobOrder.salesOrder
        ...
        allSalesOrders.forEach((salesOrder) => {
            const salesOrderId = typeof salesOrder._id === "string"
                ? salesOrder._id
        ...
        // Add ads spending
        const adsTotal = parseFloat(googleAdsSpending || 0) +
