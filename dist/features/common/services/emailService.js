 ...
 */
export const sendEmailWithS3PDF = async (documentData, documentType, documentId, s3PdfUrl) => {
    try {
        ...
        try {
            ...
            throw new Error(`Failed to download PDF from S3: ${downloadError.message}`);
        }
        const emailOptions = {
        ...
        if (error.code === "ENOTFOUND") {
            ...
            throw new Error(`Cannot access PDF at ${s3PdfUrl}. PDF file may not have public read permissions.`);
        }
        else {
            throw error;
        }
 ...
 * @returns {Promise<boolean>} - Success status
 */
export const sendSalesOrderEmail = async (salesOrderData, salesOrderId, s3PdfUrl, paymentLinkUrl = null) => {
    ...
    const emailDataWithPaymentLink = paymentLinkUrl
        ...
        : salesOrderData;
    return sendEmailWithS3PDF(emailDataWithPaymentLink, "salesOrder", salesOrderId, s3PdfUrl);
};
/**
 ...
 * @returns {Promise<boolean>} - Success status
 */
export const sendInvoiceEmail = async (invoiceData, salesOrderId, s3PdfUrl, paymentLinkUrl) => {
    if (!paymentLinkUrl) {
    ...
    return sendEmailWithS3PDF(emailDataWithPaymentLink, "invoice", salesOrderId, s3PdfUrl);
};
