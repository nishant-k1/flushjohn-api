...
export const findRecentPricing = async ({ zipCode, city, state, eventType, quantity, limit = 10, }) => {
    const query = {};
    if (zipCode) {
...
export const calculateAveragePricing = async ({ zipCode, city, state, eventType, quantity, }) => {
    const query = {};
    if (zipCode) {
        query.zipCode = zipCode;
    }
    ...
    const totalPricePerUnit = pricingHistory.reduce((sum, record) => sum + record.pricePerUnit, 0);
...
//# sourceMappingURL=vendorPricingRepository.js.map