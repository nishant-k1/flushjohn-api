...
export const updateOnSalesOrderCreated = async (leadId, salesOrderId) => {
    return await ConversationLog.updateMany({ lead: leadId }, {
...
export const markAsProcessed = async (id, extractedLearnings) => {
    return await ConversationLog.findByIdAndUpdate(id, {
        extractedLearnings,
        processed: true,
...
export const findSuccessfulConversations = async (options = {}) => {
    ...
    return await ConversationLog.find(query)
        .sort({ createdAt: -1 })
        .limit(limit)
        .select("transcript extractedInfo quotedPrice pricingBreakdown successfulResponses effectiveTactics");
};
/**
...
//# sourceMappingURL=conversationLogRepository.js.map