 ...
 */
export const analyzePronunciation = async ({ audioChunk, transcript, confidence, wordLevelConfidence = [], }) => {
    try {
        ...
        const wordAnalysis = await analyzeWordLevelPronunciation(transcript, wordLevelConfidence);
        // Analyze syllables
        ...
        const adjustments = calculateAdjustments(wordAnalysis, syllableAnalysis, phoneticAnalysis);
        // Final score calculation
        const finalScore = Math.min(5, Math.max(1, baseScore + adjustments.totalAdjustment));
        return {
...
export const calculateOverallScore = (segments) => {
    ...
    const phoneticScores = segments
        ...
        .filter((s) => s > 0);
    const syllableAccuracy = syllableScores.length > 0
        ? syllableScores.reduce((sum, s) => sum + s, 0) / syllableScores.length
        : 3.0;
    const phoneticAccuracy = phoneticScores.length > 0
 ...
 */
const analyzeWordLevelPronunciation = async (transcript, wordLevelConfidence) => {
    ...
    const avgScore = wordScores.reduce((sum, w) => sum + w.score, 0) / wordScores.length;
    return {
        words: wordScores,
...
const analyzeSyllables = async (transcript) => {
    ...
    const accuracy = multiSyllableWords.length > 0
...
const analyzePhonetics = async (transcript) => {
    ...
    const accuracy = issues.length > 0 ? Math.max(2.5, 5 - issues.length * 0.3) : 4.0;
    return {
        words: words,
        issues: issues,
        accuracy: Math.round(accuracy * 10) / 10,
 ...
 */
const calculateAdjustments = (wordAnalysis, syllableAnalysis, phoneticAnalysis) => {
 ...
 */
export const generateRecommendations = async (overallScore, segments, fullTranscript) => {
...
//# sourceMappingURL=pronunciationAnalysisService.js.map