...
const getOpenAIClient = () => {
    if (!openai) {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error("OPENAI_API_KEY environment variable is required for AI sales assistance");
        }
        openai = new OpenAI({
 ...
 */
export const getVendorPricing = async ({ zipCode, city, state, eventType, quantity = 1, productItem = "Standard Porta Potty", // Default product item for sales assist
 }) => {
    try {
        let query = {};
        if (zipCode) {
        ...
        if (!vendors || (Array.isArray(vendors) && vendors.length === 0)) {
            return {
                ...
                message: "No vendors found for this location. You may need to search manually.",
            };
        }
        ...
        try {
            ...
            if (aiSuggestedRate.dataSources) {
                historicalData = {
                    ...
                    message: aiSuggestedRate.dataSources.historicalSamples > 0
            ...
            const eventTypeMultiplier = eventType === "construction"
                ? 0.9
                : eventType === "wedding"
                    ? 1.2
                    : 1.0;
 ...
 */
export const generateResponseSuggestion = async ({ customerQuery, extractedInfo, pricing, }) => {
    try {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error("OpenAI API key is not configured");
        }
...
Be professional, friendly, and focused on helping the customer.`;
        ...
        if (pricing) {
            ...
            if (pricing.quantity)
                userPrompt += `- Quantity: ${pricing.quantity}\n`;
            userPrompt += `- Found ${pricing.vendors?.length || 0} vendor(s) in the area\n`;
        }
...
export const submitVendorQuote = async (quoteData) => {
    try {
        const { vendorId, vendorName, zipCode, city, state, eventType, quantity, pricePerUnit, totalPrice, additionalCharges, quotedBy, notes, source, aiSuggestedPrice, } = quoteData;
        let priceDifference = null;
        let accuracyRating = null;
        if (aiSuggestedPrice) {
            priceDifference = totalPrice - aiSuggestedPrice;
            const differencePercentage = Math.abs((priceDifference / aiSuggestedPrice) * 100);
...
- Maintaining professional vendor relationships`;
        ...
        throw new Error(`Failed to generate vendor call suggestions: ${error.message}`);
    }
};
/**
 ...
 */
export const generateRealTimeResponse = async ({ transcript, conversationHistory = [], extractedInfo = {}, leadId = null, mode = "sales", }) => {
    try {
        if (!process.env.OPENAI_API_KEY) {
            throw new Error("OpenAI API key is not configured");
        }
...
//# sourceMappingURL=salesAssistService.js.map