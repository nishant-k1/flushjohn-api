...
router.post("/analyze", async (req, res) => {
    try {
        ...
        const result = await salesAssistService.analyzeConversation(transcript, context);
        res.status(200).json({
            success: true,
            data: result,
        });
...
router.post("/vendor-call-suggestions", async (req, res) => {
    try {
        ...
        const suggestions = await salesAssistService.generateVendorCallSuggestions(leadId);
        res.status(200).json({
            success: true,
            data: suggestions,
        });
...
router.post("/submit-vendor-quote", async (req, res) => {
    try {
        const { vendorId, vendorName, zipCode, city, state, eventType, quantity, pricePerUnit, totalPrice, additionalCharges, quotedBy, notes, source, aiSuggestedPrice, // Optional: compare with AI suggestion
         } = req.body;
        ...
        res.status(201).json({
            success: true,
            data: quote,
            message: "Vendor quote saved successfully. Pricing accuracy will improve.",
        });
    }
    catch (error) {
...
router.post("/vendor-conversation", async (req, res) => {
    try {
        const { transcript, speakerCount, wordCount, lineCount, duration, notes } = req.body;
        if (!transcript ||
        ...
        processVendorConversationForLearning(vendorConversation._id).catch((err) => {
...
//# sourceMappingURL=salesAssist.js.map